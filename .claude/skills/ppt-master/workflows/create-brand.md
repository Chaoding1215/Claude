...
6. **Background color and the full coordinated palette are a hard brand-wins rule, not a default to weigh against other signals** — when a brand package is loaded, page background color plus the complete coordinated color set (primary / secondary / accent / neutral tiers) come **verbatim** from that brand's Color Scheme, with zero blending from a layout template's own advisory colors (a `fidelity`-mode layout may carry a "default rendering, override freely" Color Scheme section of its own — that content is void the instant a brand is present, never partially kept) and zero supplementing from Strategist's industry-color quick reference. The only exception is the user explicitly requesting a different color for this project — Strategist's own judgment, a layout's suggested defaults, or "this industry usually uses X" are never sufficient grounds to deviate.
