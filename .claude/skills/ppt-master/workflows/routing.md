...
| Preview a real page before the full deck | User explicitly asks to see a sample page / composition / the look on a real page before generating the whole deck | [`blueprint-preview`](./blueprint-preview.md) at the start of Step 6 | `design_spec.md` + `spec_lock.md` exist | Approved pilot SVG pages, then the remaining pages in the normal continuous pass | Do not run without explicit user request; stop until the pilot is approved |
...
| Template fidelity verification | User explicitly asks to verify a fidelity/mirror deck reproduced its template | [`verify-template-fidelity`](./verify-template-fidelity.md) between Step 6 and Step 7 | SVG pages exist; deck uses a fidelity/mirror template; `svg_quality_checker` binding check passed | Per-page structural-fidelity report; collapsed pages re-drawn to template density | Do not run without explicit user request; not for standard/free-design decks; stop on structural-collapse failures until fixed |
