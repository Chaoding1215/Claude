...
**Pattern Rescue (fidelity mode, optional — before collapsing a page into an existing cluster)**: [Page Roster](../references/template-designer.md#page-roster)'s clustering rule collapses near-duplicate pages into one variant. Before absorbing a page that way, check whether it carries a distinctive sub-pattern that the collapse would erase — a hierarchy/emphasis logic, an annotation convention (footnote/caveat treatment), a color-as-signal rule, or a cross-page pacing choice, none of which show up as a "structurally distinct enough for its own roster variant" signal but may still be worth keeping. If one is found, run the [Pattern Promotion Gate](../references/pattern-promotion-gate.md) before letting the page collapse silently as usual. Design status: **new mechanism, opt-in** — see [`docs/agents/ppt-master-layout-reference.md`](../../../../docs/agents/ppt-master-layout-reference.md). Skipping this check changes nothing — the page collapses exactly as it does today.
