...
| `blueprint-preview` | [`blueprint-preview.md`](./blueprint-preview.md) | User explicitly asks to preview a real page's composition before the full deck is generated | `design_spec.md` + `spec_lock.md` exist; at Step 6 entry | Do not enter by default; not inferred from deck size/template kind | Start of Step 6, before the continuous pass | Approved pilot SVG pages in `svg_output/`; remaining pages generated after | Blocks for user approval of the pilot before the rest are drawn |
...
| `verify-template-fidelity` | [`verify-template-fidelity.md`](./verify-template-fidelity.md) | User explicitly asks to verify a fidelity/mirror deck reproduced its template | SVG pages exist; deck uses a fidelity/mirror template (`page_layouts` populated); `svg_quality_checker` binding check passed | Do not enter by default; not for standard/free-design decks; never inferred from deck size | Between Step 6 and Step 7 | Per-page structural-fidelity report; failed pages re-drawn to template density | Blocks on structural-collapse failures until fixed or exception recorded |
