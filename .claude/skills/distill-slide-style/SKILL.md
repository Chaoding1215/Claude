...
**This corpus's specific findings and decisions** (Huawei/Telefonica corpus, 6 content samples + 1 template) are documented in `reference/style-spec-v10.md` — a consolidated rewrite superseding the per-version `v2`-`v9` files (kept alongside it as the audit trail for future Reconcile passes; do not read them for current findings, only for history). Notably: the accent-color and grid-compliance findings, the Google-Slides-vs-PowerPoint authoring-tool split, and the descriptive-fidelity-with-color-override decision for the eventual apply-skill are all specific to this corpus and provisional pending more samples/template variants — re-derive them for any other corpus rather than assuming they transfer.
