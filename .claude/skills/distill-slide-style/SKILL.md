...
3. **Aggregate** — batch profiles per the batch-size limit in Reference, run `templates/aggregate_prompt.md`, and run a second aggregation pass over the batch summaries if more than one batch was needed. *Done when* `docs/style-spec-v{N}.md` exists with all four required sections and every rule in it cites a real sample filename + slide number.
